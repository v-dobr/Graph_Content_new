# <a name="get-started-with-microsoft-graph-in-an-asp.net-4.6-mvc-app"></a>Начало работы с Microsoft Graph в приложении ASP.NET 4.6 MVC
...
**Не хотите создавать приложение?** С помощью [краткой инструкции по Microsoft Graph](https://graph.microsoft.io/en-us/getting-started) вы сможете быстро приступить к работе.

Чтобы скачать версию примера приложения Connect, которая использует конечную точку Azure AD и получает доступ к Microsoft Graph с помощью вызовов REST, см. [пример Office 365 Connect ASP.NET MVC](https://github.com/microsoftgraph/aspnet-connect-rest-sample).
...
URI перенаправления выступает в роли URL-адреса зарегистрированного проекта. Запрошенные [области разрешений](https://graph.microsoft.io/en-us/docs/authorization/permission_scopes) позволяют приложению получить данные профилей пользователей и отправить сообщение электронной почты.
